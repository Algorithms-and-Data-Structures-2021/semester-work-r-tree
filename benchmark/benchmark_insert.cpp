...
int main() {
  ...
  const auto output_path = string (kProjectPath)+"/benchmark/result/benchmark_insert_result.txt";

  // работа с набором данных
  vector <string> folders = {"01", "02", "03", "04", "05", "06", "07", "08", "09", "10"};
  vector <string> files = {"100", "500", "1000", "5000", "10000", "50000", "100000", "500000", "1000000","2500000", "5000000"};
  RTree tree;
      ...
      for (int i = 1; i < 11; i++) { // Запускаем замерку времени 10 раз
        ...
        if (input_file) {
          while (!line.empty()) {
            ...
            string token2 = line.substr(3);
            vector<float> c;
            cout<<static_cast<float>(stoi(token1))<<static_cast<float>(stoi(token2));
